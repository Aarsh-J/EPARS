"""
config.py — Shared constants for all dataset generation scripts.
...
SENIOR_LEVELS      = {"Senior", "Lead", "Principal"}   # used for lead eligibility checks
...
# Each dept: (primary_skills_pool, secondary_skills_pool)
DEPT_SKILLS = {
    "Engineering": (
...
# Flat set of all skills — used for cross-dept skill validation
...
# Skills required per task type — aligned to DEPT_SKILLS pools
SKILLS_BY_TASK_TYPE = {
...
EMP_TYPE_CAPACITY = {"Full-time": 40.0, "Part-time": 20.0, "Contract": 35.0}

# ─── Misc Employee Lookups ────────────────────────────────────────────────────
...
STRESS_WEIGHTS  = [0.45, 0.35, 0.20]
LANGUAGES       = ["English", "Hindi", "Spanish", "French", "German",
                   "Mandarin", "Tamil", "Telugu", "Kannada", "Arabic"]
...
BIZ_VALUE_W    = [0.10, 0.30, 0.40, 0.20]
CUST_IMPACT    = ["None", "Minor", "Moderate", "Major"]
CUST_IMPACT_W  = [0.20, 0.25, 0.35, 0.20]
COMM_FREQ      = ["Daily", "Weekly", "Bi-weekly"]
COLLAB_TOOLS   = [
    "Slack,Jira,Confluence", "Teams,Azure DevOps", "Slack,Trello",
    "Jira,Confluence,Zoom", "Asana,Slack",
]
COMPLEXITY_BUDGET   = {"Low": (20000, 80000), "Medium": (80000, 300000),
                       "High": (300000, 800000), "Very High": (800000, 2000000)}
COMPLEXITY_DURATION = {"Low": 30, "Medium": 90, "High": 180, "Very High": 270}
COMPLEXITY_RISK_BASE= {"Low": 20, "Medium": 40, "High": 60, "Very High": 75}
...
COMPLEXITY_TECH_SCORE = {"Simple": (1, 4), "Moderate": (4, 7), "Complex": (7, 9), "Very Complex": (8, 10)}

# ─── Schedule / Workload / Burnout Lookups ────────────────────────────────────
...
def rand_datetime(d_start, d_end=None, h_lo=8, h_hi=18) -> str:
    """If d_end provided, pick a random date in [d_start, d_end], else use d_start."""
...
def perf_rating_from_score(score: float) -> str:
    """Derive performance rating deterministically from score."""
...
def get_emp_skills(emp_row) -> set:
    """Return full skill set for an employee row (primary + secondary)."""
    skills = set()
