...
| `collaborative_history_score` | Recomputed from `employees.past_team_members` | Pairwise overlap: fraction of member-pairs who have previously worked together. The dataset-generated value is incorrect (uses individual collaboration score average, not actual co-team history) — must be recomputed during feature extraction |
...
### 4.4 Budget Fit (from `projects`)

| Feature | Why included |
|---|---|
| `budget_per_head` | Project budget divided by team size — sets context for whether the team size is proportionate to the project investment |
...
**Dropped:** `avg_team_compatibility` — 35% of the source column (`team_compatibility_score`) is `Normal(70,15)` random noise by dataset generator design. Averaging noise across members adds no signal.
...
**Dropped:** `proj_delay_risk`, `proj_budget_overrun_risk`, `proj_scope_creep` — all three are derived from execution-state columns (`overdue_milestones`, `consumed_resources`, `days_ahead_behind`) that are near-zero at team formation time. They carry no predictive signal about team quality before the project runs.
...
| `predicted_success_rate` | **Circular feature** — it is a weighted combination of `skill_diversity`, `experience_balance`, `collaborative_history`, `workload_balance`, and `avg_seniority`, all of which are already separate features in the model. Including it alongside its own components causes multicollinearity |
...
| `current_salary` | Not used as a feature — dropped to avoid proxy discrimination |
| `past_team_members` | Not a direct feature — used to recompute `collaborative_history_score` via pairwise overlap during feature extraction |
...
| `resource_consumption_ratio` | `consumed_resources ≈ 0` at formation time; ratio is meaningless |
| `milestone_completion_rate` | `completed_milestones = 0` at formation time; ratio is meaningless |
